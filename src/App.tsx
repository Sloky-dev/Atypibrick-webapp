...
function App() {
  ...
  const [page, setPage] = useState<'collection' | 'rooms'>('collection')
  const [locateSet, setLocateSet] = useState<LegoSet | null>(null)
  ...
  }, [hasMore, loadMore, loading, page])
  useEffect(() => { const logout = () => { setUser(null); setPage('collection'); setLocateSet(null) }; window.addEventListener('atypibrick:unauthorized', logout); return () => window.removeEventListener('atypibrick:unauthorized', logout) }, [])
  ...
  const logout = () => { void authApi.logout().finally(() => { clearSession(); setUser(null); setSets([]); setPage('collection'); setLocateSet(null) }) }
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${menuOpen ? 'open' : ''}`}>
      ...
      <nav className="sidebar-nav" aria-label="Navigation principale">
        ...
        <a className={page === 'collection' ? 'active' : ''} href="#collection" onClick={() => { setPage('collection'); setMenuOpen(false) }}><Library /> Ma collection</a>
        <button type="button" className={page === 'rooms' ? 'active' : ''} onClick={() => { setPage('rooms'); setLocateSet(null); setMenuOpen(false) }}><MapPin /> Brick Room</button>
