...
export function LoginPage({ onLogin }: { onLogin: (user: User) => void }) {
  ...
  return <main className="login-page">
    ...
    <form className="login-card" onSubmit={async (event) => { event.preventDefault(); setLoading(true); setError(''); try { const result = await authApi.login(email, password); localStorage.setItem('atypibrick_token', result.accessToken); onLogin(result.user) } catch (reason) { setError(reason instanceof Error ? reason.message : 'Connexion impossible') } finally { setLoading(false) } }}>
