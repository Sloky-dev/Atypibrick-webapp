...
const statusLabel: Record<Room['status'], string> = { capture: 'Capture', queued: 'En attente', processing: 'Assemblage', ready: 'Visite disponible', failed: 'À reprendre' }
const roomStatus = (room: Room) => room.captureKind === 'legacy' ? 'Passer au panorama' : statusLabel[room.status]
...
function RoomDetail({ initialRoom, capabilities, initialMarker, onBack, onUpdated }: { initialRoom: Room; capabilities: RoomCapabilities; initialMarker: string | null; onBack: () => void; onUpdated: () => void }) {
  ...
  async function saveMarker() {
    ...
    await mutate(async () => {
      const marker = repositioning ? await brickRoomApi.reposition(room.id, repositioning, label.trim(), position) : await brickRoomApi.createMarker(room.id, label.trim(), position)
      setRepositioning(null)
      setSelectedId(marker.id); setPosition(null); setPlacing(false); setLabel('')
    })
  }
  async function deleteRoom() {
    if (!confirm(`Supprimer « ${room.name} », ses photos, son panorama et ses repères ? Les sets resteront dans votre collection.`)) return
    setBusy(true)
    try {
  ...
  async function reset() {
    if (!confirm('Remplacer la capture ? Les anciennes photos seront supprimées. Les sets associés aux emplacements sont conservés ; les repères devront être replacés.')) return
    setBusy(true); setError('')
    try {
      const value = await brickRoomApi.reset(room.id)
      for (const capture of await captureStore.list(room.id)) await captureStore.remove(capture.id)
      setMarkers((current) => current.map((marker) => ({ ...marker, positioned: false })))
      setPlacing(false); setRepositioning(null); setPosition(null)
      updateRoom(value)
    }
    catch (e) { setError(errorMessage(e)) }
    finally { setBusy(false) }
  ...
  return <section className="room-detail"><button className="room-back" onClick={onBack}><ArrowLeft size={16} /> Mes pièces</button><div className="room-heading"><div><span className="eyebrow">BRICK ROOM</span><h1>{room.name}</h1><p>{roomStatus(room)} · {room.photoCount} photos</p></div><button className="button ghost" disabled={busy || room.status === 'processing'} onClick={() => void deleteRoom()}><Trash2 size={16} /> Supprimer</button></div>
    ...
    {room.captureKind === 'panorama' && (room.status === 'capture' || room.status === 'failed') && <RoomCapture room={room} capabilities={capabilities} onChange={updateRoom} />}
    {(room.status === 'queued' || room.status === 'processing') && <div className="room-processing" role="status"><div className="loader" /><h2>{room.stage}</h2><p>L’assemblage peut prendre plusieurs minutes. Vous pouvez quitter cette page et revenir plus tard.</p>{!capabilities.workerAvailable && <p>Le traitement est momentanément indisponible. Votre capture est conservée.</p>}</div>}
    {room.status !== 'processing' && room.status !== 'queued' && (room.status === 'ready' || room.captureKind === 'legacy') && <div className="room-notice"><button className="button ghost" disabled={busy} onClick={() => void reset()}>{room.captureKind === 'legacy' ? 'Créer le panorama de cette pièce' : 'Actualiser le panorama'}</button><p>Les associations avec vos sets seront conservées. Vous replacerez vos repères sur la nouvelle vue.</p></div>}
    {room.status === 'ready' && <><p className="room-notice">Visite panoramique · un point de vue fixe. Les zones non capturées restent vides.</p><div className="room-workspace"><div><Suspense fallback={<div className="room-viewer" role="status">Ouverture de la visite…</div>}><RoomViewer roomId={room.id} markers={markers} selectedId={selectedId} placing={placing} onPosition={setPosition} onSelect={setSelectedId} /></Suspense>
      <div className="room-toolbar"><button className={`button ${placing ? 'ghost' : 'primary'}`} onClick={() => { setPlacing(!placing); setPosition(null); setRepositioning(null); setLabel('') }}><MapPin size={16} />{placing ? 'Annuler le placement' : 'Ajouter un emplacement'}</button></div>
      {placing && <form className="room-marker-form" onSubmit={(e) => { e.preventDefault(); void saveMarker() }}><label>Nom de l’emplacement<input value={label} maxLength={100} required placeholder="Vitrine · Étagère du haut" onChange={(e) => setLabel(e.target.value)} /></label><p>{position ? 'Position choisie. Vous pouvez toucher un autre point pour la modifier.' : 'Touchez un point du meuble dans le panorama.'}</p><button className="button primary" disabled={!position || !label.trim() || busy}>Enregistrer le repère</button></form>}
    </div><aside className="room-locations"><h2>Mes emplacements</h2>{!markers.length && <p>Ajoutez un repère sur le meuble, puis associez-lui les sets qui y sont rangés.</p>}<div className="room-marker-list">{markers.map((marker) => <button className={selectedId === marker.id ? 'selected' : ''} key={marker.id} onClick={() => setSelectedId(marker.id)}><MapPin size={16} /><span>{marker.label}{!marker.positioned && <small>À replacer sur le panorama</small>}<small>{marker.sets.length} set{marker.sets.length > 1 ? 's' : ''}</small></span></button>)}</div>
      {selected && <div className="room-selected"><button className="button ghost" disabled={busy} onClick={() => { setRepositioning(selected.id); setPlacing(true); setLabel(selected.label); setPosition(null) }}>Replacer cet emplacement</button><div className="room-toolbar"><h3>{selected.label}</h3><button aria-label="Supprimer cet emplacement" disabled={busy} onClick={() => { if (confirm('Supprimer ce repère et ses associations ? Les sets restent dans la collection.')) void mutate(() => brickRoomApi.deleteMarker(room.id, selected.id)) }}><Trash2 size={16} /></button></div>
...
export function BrickRooms({ locateSet }: { locateSet: LegoSet | null }) {
  ...
  const [capabilities, setCapabilities] = useState<RoomCapabilities>({ workerAvailable: false, minPhotos: 3, maxPhotos: 36 })
  ...
  return <section className="brick-rooms"><div className="room-heading"><div><span className="eyebrow">VOTRE COLLECTION, À SA PLACE</span><h1>Brick Room<span>.</span></h1><p>Capturez votre espace. Visitez-le en panorama. Retrouvez chaque set.</p></div><Box size={54} /></div>
    ...
    <div className="room-intro"><Camera size={28} /><div><h2>Votre première capture</h2><p>Créez un panorama en tournant sur place, ou importez une photo 360°. Ajoutez des repères sur vos meubles et associez les sets qui y sont rangés.</p></div></div>
