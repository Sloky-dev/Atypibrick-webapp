...
function RoomDetail({ initialRoom, capabilities, initialMarker, onBack, onUpdated }: { initialRoom: Room; capabilities: RoomCapabilities; initialMarker: string | null; onBack: () => void; onUpdated: () => void }) {
  ...
  return <section className="room-detail"><button className="room-back" onClick={onBack}><ArrowLeft size={16} /> Mes pièces</button><div className="room-heading"><div><span className="eyebrow">BRICK ROOM</span><h1>{room.name}</h1><p>{statusLabel[room.status]} · {room.photoCount} photos</p></div><button className="button ghost" disabled={busy || room.status === 'processing'} onClick={() => void deleteRoom()}><Trash2 size={16} /> Supprimer</button></div>
    ...
    {room.status === 'ready' && <><p className="room-notice">Première reconstruction en points colorés · {room.registeredImages}/{room.photoCount} photos reliées · {room.pointCount.toLocaleString('fr-FR')} points. Les dimensions ne sont pas calibrées.</p><div className="room-workspace"><div><Suspense fallback={<div className="room-viewer" role="status">Ouverture de la vue 3D…</div>}><RoomViewer roomId={room.id} markers={markers} selectedId={selectedId} placing={placing} onPosition={setPosition} onSelect={setSelectedId} /></Suspense>
      <div className="room-toolbar"><button className={`button ${placing ? 'ghost' : 'primary'}`} onClick={() => { setPlacing(!placing); setPosition(null) }}><MapPin size={16} />{placing ? 'Annuler le placement' : 'Ajouter un emplacement'}</button></div>
...
export function BrickRooms({ locateSet }: { locateSet: LegoSet | null }) {
  ...
  return <section className="brick-rooms"><div className="room-heading"><div><span className="eyebrow">VOTRE COLLECTION, À SA PLACE</span><h1>Brick Room<span>.</span></h1><p>Capturez votre espace. Explorez-le en 3D. Retrouvez chaque set.</p></div><Box size={54} /></div>
    ...
    <div className="room-intro"><Camera size={28} /><div><h2>Votre première capture</h2><p>Commencez par une étagère bien éclairée. Prenez des photos sous plusieurs angles avec votre téléphone ; Atypibrick en reconstruira une première vue en points 3D colorés.</p></div></div>
    ...
    <h2>Mes pièces</h2>{loading ? <p>Chargement…</p> : !rooms.length ? <div className="room-empty"><MapPin size={28} /><p>Votre prochain repère commence par une capture.</p></div> : <div className="room-grid">{rooms.map((room) => <button key={room.id} className="room-card" onClick={() => setSelected({ room, marker: null })}><Box size={30} /><span className={`room-status ${room.status}`}>{statusLabel[room.status]}</span><h3>{room.name}</h3><p>{room.photoCount} photos{room.pointCount > 0 && ` · ${room.pointCount.toLocaleString('fr-FR')} points`}</p><span>{room.status === 'ready' ? 'Explorer la pièce →' : room.status === 'capture' || room.status === 'failed' ? 'Reprendre la capture →' : 'Voir la progression →'}</span></button>)}</div>}
  </section>
}
