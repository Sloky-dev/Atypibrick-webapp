...
export function CollectionFiltersPanel({ filters, summary, onChange, onClose }: Props) {
  ...
  return <div className="filters-panel">
    ...
    <div className="filters-grid">
      ...
      <label>Prix minimum (€)<input type="number" min="0" step="0.01" value={filters.priceMin} onChange={(event) => update('priceMin', event.target.value)} /></label>
      <label>Prix maximum (€)<input type="number" min="0" step="0.01" value={filters.priceMax} onChange={(event) => update('priceMax', event.target.value)} /></label>
      <label>Pièces minimum<input type="number" min="0" step="1" value={filters.partsMin} onChange={(event) => update('partsMin', event.target.value)} /></label>
      <label>Pièces maximum<input type="number" min="0" step="1" value={filters.partsMax} onChange={(event) => update('partsMax', event.target.value)} /></label>
    </div>
    <button className="filters-reset" onClick={() => onChange(emptyCollectionFilters)}><RotateCcw /> Réinitialiser les filtres</button>
  </div>
}
