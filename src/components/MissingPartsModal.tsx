...
export function MissingPartsModal({ set, onClose, onCountChange }: { set: LegoSet; onClose: () => void; onCountChange: (count: number) => void }) {
  ...
  const add = async () => {
    ...
    try { const item = await missingPartsApi.add(set.id, selected.elementId, quantity); setItems((current) => { const next = [item, ...(current || [])]; onCountChange(next.reduce((total, part) => total + part.quantity, 0)); return next }); setReference(''); setQuantity(1); setCandidates([]); setSelectedId('') }
  ...
  const remove = async (id: string) => { setSaving(true); try { await missingPartsApi.remove(id); setItems((current) => { const next = current?.filter((item) => item.id !== id) ?? []; onCountChange(next.reduce((total, part) => total + part.quantity, 0)); return next }) } catch (reason) { setError(reason instanceof Error ? reason.message : 'Suppression impossible') } finally { setSaving(false) } }
