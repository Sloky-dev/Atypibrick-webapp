...
export function MissingPartsModal({ set, onClose, onChange }: Props) {
  ...
  const replace = async (item: MissingPart) => {
    const rawPrice = window.prompt(`Prix total payé pour remplacer ${item.quantity} pièce${item.quantity > 1 ? 's' : ''} (€)`, '0')
    if (rawPrice === null) return
    const price = Number(rawPrice.replace(',', '.'))
    if (!Number.isFinite(price) || price < 0) { setError('Saisissez un prix valide.'); return }
    setSaving(true); setError('')
    try {
      const updated = await missingPartsApi.replace(item.id, price)
      setItems((current) => {
        const next = current?.map((entry) => entry.id === item.id ? updated : entry) ?? []
        onChange(missingCount(next), price)
        return next
      })
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Remplacement impossible') }
    finally { setSaving(false) }
  }
