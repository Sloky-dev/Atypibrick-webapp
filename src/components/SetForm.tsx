  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
    <form className="modal" onSubmit={async (e) => { e.preventDefault(); setSaving(true); try { await onSubmit(form) } finally { setSaving(false) } }}>
      ...
      <div className="form-grid">
        <label>Marque<select value={form.brand} onChange={(e) => setForm((current) => ({ ...current, brand: e.target.value as LegoSetPayload['brand'], setNumber: '' }))}><option>LEGO</option><option>CaDA</option><option>Jie Star</option></select></label>
        ...
        <label>Set identifié<div className={`resolved-set ${lookupError ? 'invalid' : resolvedSet ? 'valid' : ''}`}>{lookingUp ? <><LoaderCircle className="lookup-spinner" /> Recherche…</> : resolvedSet ? <><Check /> {resolvedSet.name}</> : lookupError || 'Saisissez une référence LEGO, CaDA ou Jie Star valide'}</div></label>
        ...
        <label>Condition d'achat<select value={form.condition} onChange={(e) => update('condition', e.target.value)}><option>Neuf</option><option>Occasion</option></select></label>
        ...
        <label className="gift-option"><input type="checkbox" checked={form.isSealed} onChange={(e) => setForm((current) => ({ ...current, isSealed: e.target.checked }))} /><span><strong>Set scellé</strong><small>La boîte est encore fermée avec ses scellés d’origine.</small></span></label>
